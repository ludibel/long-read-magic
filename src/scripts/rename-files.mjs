...
async function main(rootPath) {
  ...
  for (let project of projects) {
    ...
    for (let sample of samples) {
      ...
      const binFilesToRename = fs.readdirSync(binsPath)
        ...
        .map((filename) => {
          ...
          const newBinName = generateBinName(project, sample, binNumber);
          const newFilename = newBinName + ".fa";
          return { oldFilename: filename, newBinName, newFilename };
        });

      console.log(`Processing ${project} - ${sample}`)

      renameBins(binFilesToRename, binsPath);
      renameReferencesInCsv(samplePath, project, sample);
      await renameContigs(fastaPath, binsPath, project, sample);
...
function confirmation(rootPath) {
  return new Promise((resolve) => {
    ...
    rl.question(`Are you sure you want to rename files in ${rootPath}? (this path is defined in src/scripts/rename-files.mjs)  (y/n) `, (answer) => {
...
function renameBins(binFiles, binsPath) {
  binFiles.forEach(({ oldFilename, newFilename }) => {
    ``
    const binFilePath = path.join(binsPath, oldFilename);
    const newBinFilePath = path.join(binsPath, newFilename);
...
async function renameContigs(fastaPath, binsPath, project, sample) {
  await renameContigsForFile(fastaPath, project, sample);
...
async function renameContigsForFile(filepath, project, sample) {
...
function mergeTsvFiles(samplePath) {
  ...
  tsvFiles.forEach((tsvFile) => {
    ...
    for (let i = 1; i < originalCsvLines.length; i++) {
      ...
      // change all commas in tsv line to "---" so that it can be merged successfuly into a csv file
